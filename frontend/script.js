...
function startBusFacts() {
  const facts = [
    "� India has one of the largest intercity bus networks in the world.",
    "🚍 Luxury buses often feature Wi-Fi, charging points, and spacious recliners.",
    "🛣️ Bus routes connect remote towns to major cities across the country.",
    "🧳 Many bus terminals offer luggage handling and travel assistance.",
    "⏱️ Popular routes can have buses departing every 15-30 minutes."
  ];

  let index = 0;
  const factElement = document.getElementById("busFact");
...
function setupNotifications() {
  ...
  notifyBtn.addEventListener("click", () => {
    showToast("2 travel alerts: food delivery window open and bus running on time.");
  });
}
...
async function handleBusSearch(event) {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/buses?${params}`);
    const data = await response.json();
    renderBusResults(data.buses);
  } catch (error) {
    showToast("Unable to fetch buses right now.");
  }
}

function renderBusResults(trains) {
  const container = document.getElementById("busResults");
  if (!container) return;

  if (!trains.length) {
    container.innerHTML = `<div class="card empty-state">No buses found for this route.</div>`;
    return;
  }

  container.innerHTML = trains.map((train) => `
    <article class="train-card">
      <span class="pill">${train.busNumber}</span>
      <h3>${train.busName}</h3>
      ...
      <div class="train-actions">
        ...
        <button class="primary-btn" type="button" onclick='selectBus(${JSON.stringify(train)})'>Book Now</button>
      </div>
    </article>
  `).join("");
}

window.selectBus = function selectBus(train) {
  localStorage.setItem("selectedBus", JSON.stringify(train));
  document.getElementById("selectedBusId").value = train.id;
  document.getElementById("selectedBusSummary").innerHTML = `
    <h3>${train.busName}</h3>
  ...
  showToast(`${train.busName} selected for booking.`);
};
...
function handleTrainTrack(event) {
  event.preventDefault();
  const trainNumber = document.getElementById("trackBusNumber").value.trim();

  if (!trainNumber) {
    showToast("Enter a bus number.");
    return;
  }

  const delayed = Number(trainNumber.slice(-1)) % 2 === 0;
  document.getElementById("trackingResult").innerHTML = `
    <p><strong>Bus Number:</strong> ${trainNumber}</p>
...
async function handleFoodOrder(event) {
  ...
  const trainNumber = document.getElementById("foodBusNumber").value.trim();
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/food/order`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ items: cart, customerName, busNumber: trainNumber })
    });
    const data = await response.json();
...
async function handleHelpRequest(event) {
  ...
  const trainNumber = document.getElementById("helpBusNumber").value.trim();
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/help`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId: user?.id, category, busNumber: trainNumber, message })
    });
    const data = await response.json();
    ...
    document.getElementById("helpStatus").innerHTML = `
      ...
      <p><strong>Bus Number:</strong> ${data.complaint.trainNumber}</p>
    `;
    event.target.reset();
